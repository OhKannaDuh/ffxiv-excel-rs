...
impl Sheet for HousingYardObject {
    const SHEET_NAME: &'static str = "HousingYardObject";
}

impl FromExcelRow for HousingYardObject {
    fn from_row(row: &ExcelRow) -> Option<Self> {
        ...
        Some(Self {
            ...
            custom_talk_id: single_row.columns.get(5).to_u32(),
            custom_talk: RowRef::<CustomTalk>::from(single_row.columns.get(5).to_u32()),
            item_id: single_row.columns.get(6).to_u32(),
            item: RowRef::<Item>::from(single_row.columns.get(6).to_u32()),
            destroy_on_removal: single_row.columns.get(7).to_bit(0),
        })
    }
}
